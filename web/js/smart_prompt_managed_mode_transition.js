// Exact recoverable transitions for Smart Prompt's browser-owned workflow field.

const TRANSITION_FROZEN = "__smartPromptManagedPrivacyTransitionFrozen";
...
function nodeType(node) {
  return node?.comfyClass ?? node?.type ?? node?.class_type ?? null;
}

function locatorPart(value, fail) {
...
export function createSmartPromptExternalWorkflowTransition({
  app = null,
  owners,
  registerNode,
  readStorage,
  ...
  function liveGraphEntries() {
    ...
    for (let index = 0; index < pending.length; index += 1) {
      ...
      entries.push({
        graph,
        graphId: locatorPart(graph.id, fail),
        nodes: Array.isArray(graph._nodes) ? graph._nodes : graph.nodes ?? [],
      });
      pending.push(...graphValues(graph._subgraphs), ...graphValues(graph.subgraphs));
    }
    return entries;
  }
  ...
  function liveOwnerRecords(context) {
    ...
    for (const entry of liveGraphEntries()) {
      ...
      for (const node of entry.nodes) {
        if (!field.nodeTypes.includes(nodeType(node))) continue;
        registerNode(node);
        if (!owners.has(node)) fail();
        synchronizeOwner(node);
        const locator = Object.freeze({
          rootGraphId: "root",
          graphId: locatorPart(entry.graphId, fail),
          nodeId: locatorPart(node.id, fail),
        });
  ...
  function serializedGraphEntries(serialized) {
    ...
    for (const graph of definitions ?? []) {
      ...
      entries.push({ graphId: locatorPart(graph.id, fail), nodes: graph.nodes });
    }
    return entries;
  }
  ...
  function requireOwner(owner, context) {
    ...
    if (
      ...
      || !field.nodeTypes.includes(nodeType(owner.node))
    ) fail();
    return { owner, field };
  }
  ...
  return Object.freeze({
    ...
    settleModeTransition(context) {
      ...
      const settled = Promise.resolve().then(async () => {
        ...
        for (const record of before) await settleOwner(record.node, context);
    ...
    extractDetachedModeTransitionOwnerExact(owner, serialized, context) {
      ...
      if (!serializedNode || !field.nodeTypes.includes(nodeType(serializedNode))) fail();
